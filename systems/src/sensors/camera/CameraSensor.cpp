...
namespace
{
/// @brief Deterministic (seeded) noise so runs are reproducible.
std::mt19937& engine()
{
...
/// @brief Compute detection confidence from contrast, brightness, and range.
...
double computeConfidence(double contrast, double brightness, double range)
{
    ...
    {
        result = std::min(1.0, (contrast * brightness) / (range * range));
    }
    return result;
}
} // namespace
...
CameraSensor::process(const std::vector<sim::Observation>& observations) const
{
    ...
    {
        /// @brief Range gating, FOV gating, and detection threshold.
        if (obs.rangeM >= params_.minRangeM && obs.rangeM <= params_.maxRangeM &&
            std::fabs(obs.azimuthRad) <= params_.maxAzimuthFOVRad &&
            std::fabs(obs.elevationRad) <= params_.maxElevationFOVRad)
        {
            /// @brief Detection confidence from contrast/brightness (signalStrength) and range.
