...
{
/// @brief declaring relevant namespaces
...
/// @brief Base class for potential landmark models.
...
class LandmarkModel
{
  public:
    ...
    /// @brief Add a feature to the map. This is used to update the map with new features that are
    /// observed by the sensor.
    virtual bool add_feature(const RadarEcho& feature);

    /// @brief Likelihood that a given measurement fits set, given the current state of the map.
    /// This is used to compute the likelihood of a measurement given the current state of the map.
    virtual double compute_likelihood(const RadarEcho& measurement) const = 0;

    /// @brief Probability function for computation of likelihood
    virtual double set_probability_function() const = 0;

  private:
    /// @brief Map that consists of a list of features, which are the measurements that are observed
    /// by the sensor.
