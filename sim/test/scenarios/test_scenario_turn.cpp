...
/// @brief Turn scenario: aircraft flies north, turns 90 deg right, flies east.
...
int main()
{
    ...
    /// @brief --- World: landmarks around the L-shaped path ---
    sim::World world;
    /// @brief Along the north leg
    ...
    /// @brief --- Trajectory: north, turn right, east ---
    ...
    /// @brief 1. The aircraft heading changed: started north (yaw~0), ended east
    ///    (yaw~pi/2). We check the position delta between early and late steps.
    {
        /// @brief Early step: aircraft near start.
        const auto& early = steps[1];
        /// @brief Late step: aircraft near end.
        const auto& late = steps.back();
        /// @brief Should have moved both north and east.
    ...
    /// @brief 2. Landmarks were observed from multiple angles. The "corner" landmark
    ///    should appear at different azimuths as the aircraft turns.
    {
        ...
        for (const auto& s : steps)
        {
            ...
            {
                /// @brief Corner landmark is at range ~sqrt(800^2+200^2) ~= 824 m
        ...
        /// @brief Before the turn the corner is to the right (+az); after turning
    ...
    /// @brief 3. SLAM tracked pose throughout.
    for (const auto& s : steps)
    {
    ...
    /// @brief 4. Detections were produced across the scenario (radar active throughout).
