...
/// @brief Builds an aircraft trajectory from flight segments using a simple kinematic
...
{
  public:
    /// @brief startPos: NED [m]. startYaw: rad (0 = north, +east). startPitch: rad.
    TrajectoryBuilder(const core::math::Vec3& startPos, double startYaw = 0.0,
                      double startPitch = 0.0);

    /// @brief Fly straight at current heading/pitch for the given distance [m] at
    ...
    /// @brief Turn at a constant rate [rad/s] for the given angle [rad], while
    ...
    /// @brief Turn from the current heading to a target heading at a constant rate.
    TrajectoryBuilder& turnTo(double targetYawRad, double turnRateRps, double speedMps, double dt);

    /// @brief Climb/descend to a target altitude while moving forward. targetAltM is
    ...
    /// @brief Return the accumulated poses (one per timestep, including the start).
  ...
  private:
    /// @brief Integrate one timestep forward at the current heading/pitch and speed,
