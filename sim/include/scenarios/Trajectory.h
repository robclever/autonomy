...
/// @brief An aircraft trajectory: a sequence of poses indexed by timestep.
///
...
class Trajectory
{
  public:
    ...
    /// @brief Pose at the given timestep. Must hold for step < numSteps().
    virtual core::kinematics::Pose poseAt(std::size_t step) const = 0;

    /// @brief Total number of steps in the trajectory.
    virtual std::size_t numSteps() const = 0;
};

/// @brief A trajectory backed by an explicit list of poses.
