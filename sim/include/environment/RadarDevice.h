...
/// @brief A radar device mounted on the ownship (player aircraft).
///
...
class RadarDevice : public scenarios::ISensorDevice
{
  public:
    ...
    /// @brief Radar-specific output: returns the raw radar echoes.
    std::vector<systems::sensors::radar::RadarEcho>
    senseEchoes(const World& world, const core::kinematics::Pose& aircraftPose) const;

    /// @brief ISensorDevice override — returns generic Detections for the SensorSuite.
